...
public class Cards_Match : MonoBehaviour
{
    ...
    {
        cardBack = LevelManager_MatchCard.Instance.GetCardBack();
        cardFace = LevelManager_MatchCard.Instance.GetCardFace(cardValue);
    ...
    public void FlipCard()
    {
        ...
        else if (state == 0 && !do_Not) //the card is back and will be face
        {
            state = 1;
            GetComponent<Image>().sprite = cardFace;
            LevelManager_MatchCard.Instance.matchCount++;
        }
        else if (state == 1 && !do_Not) //the card is face and will be back
        {
            ...
            LevelManager_MatchCard.Instance.matchCount--;
            LevelManager_MatchCard.Instance.scoreText.text = (int.Parse(LevelManager_MatchCard.Instance.scoreText.text) - 2).ToString();
        }
    }
